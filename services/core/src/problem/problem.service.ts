...
export class ProblemService {
  ...
  async createProblem(
    ...
    try {
      const createdTestcases = await this.testcaseService.createTestcases(
  ...
  async removeTestcasesFromProblem(
    ...
    await this.problemModel.findByIdAndUpdate(problemId, {
      $pull: {
        testcases: { $in: testcaseIds.map((id) => new Types.ObjectId(id)) },
      },
    });
  }
  ...
  async getProblemWithPublicTestcases(problemId: string): Promise<Problem> {
    const problem = await this.problemModel.findById(problemId).populate({
      path: 'testcases',
      model: 'Testcase',
      transform: (doc) => {
        if (doc.isPublic) {
  ...
  async getProblemById(problemId: string): Promise<ProblemDocument> {
    const problem = await this.problemModel.findById(problemId);
    if (!problem) {
      throw new NotFoundException(`Problem with ID ${problemId} not found`);
    }
