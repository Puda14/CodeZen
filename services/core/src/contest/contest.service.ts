...
export class ContestService {
  ...
  async getContestDetails(contestId: string, userId: string): Promise<ContestDocument> {
    ...
    return this.transformContestResponse(contestId);
  }
  ...
  async getProblemForContestant(contestId: string, problemId: string, userId: string): Promise<Problem> {
    ...
    return this.transformProblemResponse(problemExists);
  }
    ...
    return {
      ...
      problems: contest.problems.map(problem => ({
        ...
        testcases: problem.testcases.map(tc =>
          tc.isPublic
            ? { _id: tc._id, score: tc.score, input: tc.input, output: tc.output, isPublic: true }
            : { _id: tc._id, score: tc.score }
        )
      }))
    };
  }
