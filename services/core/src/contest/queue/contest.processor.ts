  ...
  async process(job: Job<any>) {
    ...
    switch (job.name) {
      case 'updateStatus': {
        ...
        this.logger.log(
          `🔄 Updating contest ${contestId} status to ${newStatus}`,
        );

        try {
          const updatedContest = await this.contestService.updateContestStatus(
            contestId,
            newStatus,
          );
          ...
          if (newStatus === ContestStatus.ONGOING) {
            ...
            const formattedContest =
              await this.contestService.fetchFullContestDetails(contestId);
            ...
            const problemIds = formattedContest.problems.map((p) =>
              p._id.toString(),
            );
            ...
            if (delay > 0) {
              this.logger.log(
                `📅 Scheduling contest ${contestId} to finish in ${delay} ms`,
              );
              ...
              this.logger.warn(
                `⚠️ Contest ${contestId} end_time already passed, skipping scheduling.`,
              );
            }
          }
          ...
          this.logger.error(
            `❌ Failed to update contest ${contestId}: ${error.message}`,
          );
          throw error;
        }
        break;
